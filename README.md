...
En V4.7, las tres superficies tienen contratos distintos y explícitos: **REGISTRY** es el universo observado e histórico; **PIPELINE** contiene todas las oportunidades activas que ya superaron la barra de calidad para una candidatura seria; **TODAY** toma las primeras `MIN(10, PIPELINE)` usando exactamente el mismo ranking, y añade sólo carryovers humanos de lifecycle claramente identificados. Si una oportunidad normal aparece en PIPELINE, Career Ops está afirmando que actualmente vale la pena perseguirla. TODAY difiere por rango y capacidad, no por clase de calidad.

`TODAY_TARGET = 10` nunca diluye calidad. Si hay menos de diez filas normales, PIPELINE tiene menos de diez candidatos fuertes activos; los leads no resueltos permanecen en Registry/Research y no se promocionan como relleno. `Needs Your Attention` cuenta únicamente intervención humana actual, no filas ni holds. Diagnósticos: `npm run pipeline:diagnostics -- --trace` y `npm run today:diagnostics -- --trace`.

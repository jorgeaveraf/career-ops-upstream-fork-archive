...
En V4.8, las superficies tienen contratos disjuntos y explícitos: **REGISTRY** es todo lo observado y su historia; **STRONG POOL** es el conjunto interno de oportunidades calificadas que Career Ops considera dignas de perseguir; **TODAY** muestra los ranks globales 1–10 del Strong Pool y añade sólo carryovers humanos gobernados; **PIPELINE** muestra exclusivamente los ranks globales 11+. Si una oportunidad normal aparece en PIPELINE, Career Ops ya cree que vale la pena perseguirla y sólo está fuera de TODAY porque hay trabajos más fuertes arriba. TODAY y PIPELINE nunca duplican una oportunidad normal.

`TODAY_TARGET = 10` nunca diluye calidad. PIPELINE vacío no implica que no existan candidatos potenciales: puede significar que todos los trabajos fuertes caben en TODAY mientras Career Ops todavía califica el Registry. SETTINGS distingue Strong Pool, Pipeline waiting y Qualification/Evaluation Backlog. **RESEARCH es infraestructura interna y está oculta**; el operador no administra esa cola. Sólo `STRONG_POOL_EXHAUSTED` significa que el universo relevante fue resuelto y no queda backlog. `Needs Your Attention` cuenta únicamente intervención humana actual, no filas ni holds. Diagnósticos: `npm run qualification:drain`, `npm run pipeline:diagnostics -- --trace` y `npm run today:diagnostics -- --trace`.

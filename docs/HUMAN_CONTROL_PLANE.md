...
In V4.7, `PIPELINE` is the ranked set of active strong pursuit candidates, and normal `TODAY` rows are exactly its first ten with identical ranks. Governed verification, answer, approval, review, and background-work carryovers are appended with lane `HUMAN_CARRYOVER` and no fabricated Pipeline rank. Stage, Status, and Action provide the primary mental model; Attention Type, Reason, Question, and Allowed Actions provide exact context and do not rely on color. The Apps Script source adds exactly two **Career Ops** actions: **Sync Jobs** and **Sync Communities**.

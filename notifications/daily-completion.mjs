...
export class DailyCompletionSummaryService{
  ...
  buildMetrics(run){
    ...
    return{localDate:date,completedLocal:formatHumanTime(run.finishedAt||run.startedAt,{timeZone:this.timeZone,now:this.clock()}),dailyStatus:run.status==='PARTIAL'?'COMPLETED_WITH_LIMITATIONS':'COMPLETED',reviewed:Number(run.summary?.ranking?.activeSet||run.summary?.discovery?.observations||0),newJobs:Number(run.jobsFound||run.summary?.discovery?.newJobs||0),passed:Number(run.eligible||run.summary?.eligibility?.eligible||0),shortlisted:Number(run.shortlisted||run.summary?.ranking?.shortlisted||0),evaluated:Number(run.evaluationsCompleted||run.summary?.evaluation?.completed||0),promoted:Number(run.summary?.evaluation?.apply||0),strongPool:Number(projection.diagnostics?.strongPool?.admittedCount||projection.diagnostics?.pipeline?.admittedCount||0),pipelineWaiting:Number(projection.diagnostics?.pipeline?.waitingCount||0),qualificationBacklog:Number(projection.diagnostics?.today?.qualificationBacklog?.pendingCandidates||0),todayCurated:Number(projection.diagnostics?.today?.todayCuratedFromPipeline||0),humanCarryovers:Number(projection.diagnostics?.today?.humanCarryovers||0),today:todayRows.length,attention:Number(data.attentionCounts?.total||0),topThree:todayRows.filter(row=>row.Lane==='CURATED_PIPELINE').slice(0,3).map(row=>({rank:row.Rank,company:row.Company,role:row.Role,stage:row.Stage,recommendation:row.Recommendation||'UNKNOWN'})),applicationsConfirmed:applications,unresolvedFailures:unresolved,limitedSources:errors.filter(value=>value.severity==='INFO').length,systemStatus:data.operationalSummary?.overall||'UNKNOWN'};
  }
